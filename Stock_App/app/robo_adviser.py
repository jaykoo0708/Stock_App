...
if __name__ == '__main__': # only execute if file invoked from the command-line, not when imported into other files, like tests
    ...
    # result = daily_prices[max(daily_prices, key=lambda v:daily_prices[v]["2. high"])]
    # max_capacity = max([i["2. high"] for i in daily_prices.values()])
    # result = max(daily_prices, key=lambda v: int(daily_prices[v]["2. high"]))
    # # print(max_capacity)
    #
    #
    # for k, nested in daily_prices:
    #     print(max(nested["2. high"].values()))

    # result = max(daily_prices.values(), key=lambda v:v["2. high"])
    # print(result)
